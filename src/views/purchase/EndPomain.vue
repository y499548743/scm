  ...
  <div>
    ...
    <el-table :data="tableData" stripe style="100%">
      ...
      <el-table-column prop="opp" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" class="btn" @click="endPomain(scope.$index, scope.row)">了结</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  created() {
    requestGet("/api/main/purchase/pomain/show", { type: 4, payType: 1 }).then(
      resp => {
  ...
  methods: {
    ...
    endPomain(index, row) {
      this.$confirm("是否继续了结采购单?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(() => {
          requestGet("/api/main/purchase/pomain/end", {
            poId: row.poId,
            payType: row.payType
          }).then(resp => {
            let res = resp.data;
