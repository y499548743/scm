  ...
  <div>
    ...
    <el-table :data="tableData" stripe style="100%">
      ...
      <el-table-column prop="opp" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" class="btn" @click="receipt(scope.$index, scope.row)">收款</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  created() {
    requestGet("/api/main/sell/somain/show", { type: 3, payType: 1 }).then(
      resp => {
      ...
      this.$confirm("该销售单是否继续收款登记?", "提示", {
        ...
        .then(() => {
          if (row.payType == 3 && row.status == 1) {
            requestPost("api/main/finance/receipt", {
              soId: row.soId,
              type: 2
            }).then(resp => {
              let res = resp.data;
