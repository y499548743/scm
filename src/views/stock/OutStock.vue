  ...
  <div>
    ...
    <el-table :data="tableData" stripe style="100%">
      ...
      <el-table-column prop="opp" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" class="btn" @click="PomainOutstock(scope.$index, scope.row)">出库</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  created() {
    requestGet("/api/main/sell/somain/show", { type: 2, payType: 1 }).then(
      resp => {
  ...
  methods: {
    ...
    PomainOutstock(index, row) {
      console.log(row, 111);
      this.$confirm("该销售单是否继续出库?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(() => {
          requestPost("api/main/stock/outstock", {
            soId: row.soId,
            payType: row.payType
          }).then(resp => {
            let res = resp.data;
