  ...
  <div>
    ...
    <el-table :data="tableData" stripe style="100%">
      ...
      <el-table-column prop="opp" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" class="btn" @click="PomainInstock(scope.$index, scope.row)">入库</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  created() {
    requestGet("/api/main/purchase/pomain/show", { type: 2, payType: 1 }).then(
      resp => {
  ...
  methods: {
    ...
    PomainInstock(index, row) {
      console.log(row, 111);
      this.$confirm("该采购单是否继续入库?", "提示", {
