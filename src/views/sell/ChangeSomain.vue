  ...
  <div>
    ...
    <table class="somainTab">
      <tr>
        ...
        <td>客户</td>
        <td>
          <el-input v-model="somainTab.customerName"></el-input>
      ...
      <tr>
        <td>创建用户</td>
        <td>
          <el-input v-model="somainTab.account" :disabled="true"></el-input>
        </td>
        <td>附加费用</td>
        <td>
        ...
        <td>销售单总价</td>
        <td>
          <el-input v-model="somainTab.soTotal" :disabled="true"></el-input>
        </td>
      </tr>
      <tr>
        <td>付款方式</td>
        <td>
          <el-select :value="somainTab.payType|paytype" placeholder="请选择付款方式" @change="paySelect">
        ...
        <td>备注</td>
        <td>
          <el-input v-model="somainTab.remark"></el-input>
        </td>
      </tr>
    </table>
    ...
    <!-- <el-dialog title="客户选择" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        <el-form-item label="客户名称">
          <el-select v-model="form.name" placeholder="请选择客户名称" @change="selectName">
            <el-option
        ...
        <el-form-item label="客户编号">
          <el-input v-model="form.customerCode" class="vcipt"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  created() {
    ...
    requestGet("/api/main/sell/somain/show", { type: 1 }).then(resp => {
      this.somainTab = resp.data.list[poIndex];
      console.log(this.somainTab, "111");
    }),
      requestGet("/api/main/sell/somain/queryItem", { soId: soId }).then(
        resp => {
  ...
  methods: {
    ...
    saveSomain() {
      ...
      requestPostJson("/api/main/sell/somain/update", this.somainTab).then(
        resp => {
          let res = resp.data;
          console.log("响应", res);
          if (res.code == 2) {
    ...
    getsoTotal() {
      this.somainTab.tipFee = Number(this.somainTab.tipFee);
      let sum = 0;
      for (let i = 0; i < this.tableData.length; i++) {
