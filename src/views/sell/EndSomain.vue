<template>
  <div>
    ...
    <el-table :data="tableData" stripe style="100%">
      ...
      <el-table-column prop="customerName" label="客户" width="100px">
        <template slot-scope="scope">
      ...
      <el-table-column prop="opp" label="操作">
        <template slot-scope="scope">
          <el-button type="primary" class="btn" @click="endSomain(scope.$index, scope.row)">了结</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  created() {
    requestGet("/api/main/sell/somain/show", { type: 4, payType: 1 }).then(
      resp => {
  ...
  methods: {
    checkTab(tab) {
      ...
      if (tab.name == "second") {
        console.log(222);
        requestGet("/api/main/sell/somain/show", { type: 4, payType: 2 }).then(
          resp => {
            this.result = resp.data;
            this.tableData = resp.data.list;
            console.log(this.tableData);
      ...
      if (tab.name == "third") {
        console.log(333);
        requestGet("/api/main/sell/somain/show", { type: 4, payType: 3 }).then(
          resp => {
            this.result = resp.data;
            this.tableData = resp.data.list;
            console.log(this.tableData);
    ...
    goPage(page) {
      ...
      if (this.activeName == "first") {
        requestGet("/api/main/sell/somain/show", {
          type: 4,
          payType: 1,
          page: page
        }).then(resp => {
    ...
    endSomain(index, row) {
      this.$confirm("是否了结?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      })
        .then(() => {
          requestGet("/api/main/sell/somain/end", {
            soId: row.soId,
            payType: row.payType
          }).then(resp => {
            let res = resp.data;
