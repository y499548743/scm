<template>
  <div>
    ...
    <el-table :data="mainData" stripe style="100%">
      <el-table-column prop="total" label="出库单据数">
      ...
      <el-table-column prop="productTotal" label="出库产品总金额">
        <template slot-scope="scope">
    ...
    <el-table :data="detailData" stripe style="100%">
      ...
      <el-table-column prop="stockTime" label="出库日期">
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      console.log(date);
      requestGet("/api/main/report/outstock/main", { time: date }).then(resp => {
        let res = resp.data;
        console.log(res);
    ...
    goPage(page) {
      let date = this.dateValue;
      this.currPage = page;
      requestGet("/api/main/report/outstock/main", {
        time: date,
        page: page
      }).then(resp => {
        let res = resp.data;
